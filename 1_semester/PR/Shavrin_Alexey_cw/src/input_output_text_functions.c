...
void userChoice(struct Text *text){
	...
	while (choice[0] != L'5'){
		...
		wprintf(L"6 - Напечатать слова встречающиеся только 2 раза.\n");
		...
		switch (choice[0]){
			...
			case L'6':
				//wprintf(L"start\n");
				s = duplicateWord(text);
				qsort(s -> wwords, s -> count_w, sizeof(struct Word), cmp);
				for (int i = 0; i < s -> count_w; i++){
					fputws(s -> wwords[i].wchars, stdout);
					wprintf(L" ");
				}
				wprintf(L"\n");
				break;
