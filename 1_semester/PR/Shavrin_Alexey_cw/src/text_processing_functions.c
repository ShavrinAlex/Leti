...
struct Sentence *duplicateWord(struct Text *text){
	int size = MEM_STEP;
	struct Sentence *s = malloc(sizeof(struct Sentence));
	int i = 0;
	struct Word *arr = malloc(size*sizeof(struct Word));
	for (struct Sentence *sent = text -> sents; sent < text -> sents + text -> count_s; sent++){
		for (struct Word *word = sent -> wwords; word < sent -> wwords + sent -> count_w; word++){
			int count = 0;
			for (struct Sentence *s = text -> sents; s < text -> sents + text -> count_s; s++){
				for (struct Word *w = s -> wwords; w < s -> wwords + s -> count_w; w++){
					if (!wcscasecmp(word -> wchars, w -> wchars))
						count++;
					if (count > 2)
						break;
				}
			...
			if (count == 2){
				int f = 0;
				for (int j = 0; j < i; j++){
					if (!wcscasecmp((arr+j) -> wchars, word -> wchars)){
						f++;
						break;
					}
				}
			...
			if (i == size){
				size += MEM_STEP;
				arr = realloc(arr, size*sizeof(struct Word));
			}
		}
	}
	s -> wwords = arr;
...
int cmp(const void *x1, const void *x2){
	return ((struct Word*)x2)->count_c - ((struct Word*)x1)->count_c;
}
