...
void * thread1() {  // функция, которую будет выполнять первая нить.
   printf("Thread_1 is started tid = %d\n", gettid());  // выводим сообщение о том, что первая нить начала работу.
   time_t timer;
   timer = time(NULL);
   int n = 1;
...
void * threadn() {  // функция, которую будет выполнять вторая нить.
   printf("Thread_n is started tid = %d\n", gettid());  // выводим сообщение о том, что вторая нить начала работу.
   time_t timer;
   timer = time(NULL);
   int n = 1;
