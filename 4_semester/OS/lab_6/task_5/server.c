...
int readFix(int sock, char* buf, int bufSize, int flags) {
    ...
    int res=recvfrom(sock,&msgLength,sizeof(unsigned),flags|MSG_WAITALL );
    if (res <= 0) return res;
    if(res > bufSize) {
    ...
    return recvfrom(sock, buf, msgLength, flags | MSG_WAITALL);
}
int sendFix(int sock, char* buf, int flags) {
    // шлем число байт в сообщении
    unsigned msgLength = strlen(buf);
    int res = sendo(sock, &msgLength, sizeof(unsigned), flags );
    if (res <= 0)
        return res;
    sendto(sock, buf, msgLength, flags);
}