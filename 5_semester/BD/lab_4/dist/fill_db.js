...
export async function fill_db() {
    ...
    //fill Breeds
    let breeds = faker.helpers.multiple(createBreed, { count: COUNT });
    //console.log(breeds)
    ...
    //fill owners
    let owners = faker.helpers.multiple(createOwner, { count: COUNT });
    //console.log(owners)
    ...
    //fill club numbers
    let club_numbers = faker.helpers.multiple(createClubNumber, { count: COUNT });
    ...
    let dogs_experts_estimates = faker.helpers.multiple(createDogExpertEstimate, { count: COUNT });
...
function createClubNumber() {
    return {
        'dog_number': faker.number.int({ min: 1, max: COUNT }),
        'club_id': faker.number.int({ min: 1, max: COUNT })
    };
}
function createDogExpertEstimate() {
    return {
        'dog_number': faker.number.int({ min: 1, max: COUNT }),
