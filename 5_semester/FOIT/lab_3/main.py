...
def calculate_stop_i(N: int, delta_f: float, frequency: list) -> int:
    """This function calculated stop i"""
...
def main():
    ...
    spectre_module = list(map(abs, np.fft.fft(signal)))
    frequency = []
    stop_i = calculate_stop_i(N, delta_f, frequency)
    H = [abs(calculate_h(L1, L2, C1, C2, R1, R2, R3, R4, j, omega)) for omega in frequency[1:]]
    H_ans = abs(calculate_h(L1, L2, C1, C2, R1, R2, R3, R4, j, 10)) # у первой гармоники ω = 10 рад/c
