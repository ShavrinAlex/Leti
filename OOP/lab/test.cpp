...
int main()
{
    ...
    Texture herotexture;
    herotexture.loadFromFile("images/hero/hero1.png");
    ...
    if ((Keyboard::isKeyPressed(Keyboard::Right) || (Keyboard::isKeyPressed(Keyboard::D)))) { //если нажата клавиша стрелка влево или англ буква А
			...
			herosprite.setTextureRect(IntRect(150 * int(CurrentFrame), 280, 150, 80)); //проходимся по координатам Х. получается начинаем рисование с координаты Х равной 0,96,96*2, и опять 0
    ...
    //if ((Keyboard::isKeyPressed(Keyboard::Right) || (Keyboard::isKeyPressed(Keyboard::D)))) { herosprite.move(0.1*time, 0); herosprite.setTextureRect(IntRect(150, 285, 150, 75)); } // см коммент выше
